...
directoryPath <- "cd /home/hallie/Desktop/SalmonAqcClimateForGithub"
plinkPath <- "/home/hallie/miniforge3/bin/plink"
...
plinkFst <- function(cluster) {
  ...
  system(paste0("/home/hallie/miniforge3/bin/plink --bfile Panel/Ssal.31.220K.mapped.June2023merge --allow-extra-chr --maf 0.01 --keep PLINK/",
                cluster, 
...
#  BIOvar_sd <- paste0(BIOvar, ".scale")
  ...
  Env2 <- updatedEnv %>%
    select(tidyselect::starts_with(c("Bio", "MS_")))
...
#  which(is.na(Env_SDmatrix))
 ...
 # Env_SDmatrix2 <- data.table(Env_SDmatrix)
  ...
  #PCAdapt
  pcadapt.object <-  read.pcadapt(paste0("wild.env.all.", cluster, ".bed"), type = "bed")
  ...
  #genos.dose <- fread(paste0("wild.env.all.", cluster, ".raw"), sep = " ") %>%  select (-FID, -IID, -PAT, -MAT, -SEX, -PHENOTYPE)
...
chrom_map <- fread(paste0("wild.env.", "all.NA", ".bim")) %>%
  mutate(CHROM = V1, SNP = V2, BP = V4) %>%
  select(CHROM, SNP, BP)

rda.loadings.map <- RDA_scores_map(rda1, chrom_map, 3)
...
write.table(l, "Figs/Supplements/EnvLoadings.tsv", row.names = T)
```

## Outlier selection - 3 SD Overlapping
...
## PCA at climate-associated - used?

```{r}
for (cluster in c("NFL", "LAB", "STL", "NA")) {
system(paste0("/home/hallie/miniforge3/bin/plink --allow-extra-chr --maf 0.01 --bfile Panel/Ssal.31.220K.mapped.June2023merge --extract PLINK/rda.outlier.bed --keep PLINK/", cluster, ".AQ.tsv --make-bed --out PLINK/", cluster, ".AQ.ClimateAsSNPs"))
}
```
...
plinkFstClimate<- function(cluster) {
  ...
  system(paste0("/home/hallie/miniforge3/bin/plink --allow-extra-chr --maf 0.01 --bfile Panel/Ssal.31.220K.mapped.June2023merge --extract PLINK/rda.outlier.bed --keep PLINK/", cluster, ".AQ.tsv --make-bed --fst --within PLINK/", cluster,".AQ.tsv --out PLINK/", cluster, ".AQ.Climate"))
...
```{r}
system(paste0("bedtools intersect -b PLINK/overlappingOLwithEstuary.bed -a Panel/Ssal3_1.genes.unique.bed -wb >  Figs/Supplements/fsatAndRda.outlier.genes.tsv"))
```
